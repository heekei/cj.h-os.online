	...
	*/
		...
		$('body').on('click','.upload_btn',function(){
			...
			wxJs.chooseImage(num-nownumber,function(data){
				...
				for(var i= 0;i<data.length;i++){
					imgstr += '<li class="weui-uploader__file"><img src="'+data[i][0]+'"></li><input value="'+data[i][1]+'" type="hidden" name="images[]">';
				}
	...
	common.createUrl = function(pstr,acstr,dostr){
		...
		return window.sysinfo.siteroot+'app/index.php?i='+window.sysinfo.uniacid+'&c=entry'+str+'&m=weliam_merchant';
	};
	...
	common.indexInit = function(){
		...
		$('.app-card').bind('click',function(){
			...
			common.confirm('提示','兑换此券需消耗'+postdata.needcredit+'个人积分,确定兑换吗？',function(){
				common.Http('post','html',common.createUrl('ajaxdeal','exchangecard'),postdata,function(data){
					if(data == '已达兑换上限'){
						thisclass.unbind();
					}
					common.alert(data);
